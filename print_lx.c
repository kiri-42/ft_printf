...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/15 17:59:48 by tkirihar          #+#    #+#             */
/*   Updated: 2021/10/15 23:32:26 by tkirihar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	print_minus_flag(t_flags *flags, int digit, unsigned int arg)
{
	...
	putlen += print_nbr_lhex(arg, 1, flags->accurasy - digit);
...
static int	print_zero_flag(t_flags *flags, int digit, unsigned int arg)
{
	...
	if (flags->sharp == 1 && arg != 0)
		putlen += print_str("0X", 1, 2);
	putlen += print_nbr_lhex(arg, 1, flags->field - digit);
	return (putlen);
}
